...
async function downloadResource(url) {
    ...
    try {
        ...
        // Handling for text-based MIME types
        if (mimeType.includes('text/')) {
            const textContent = Buffer.from(dataToWrite).toString('utf8');

            if (mimeType.includes('/css')) {
                dataToWrite = await replaceCssUrlsAndGetContent(textContent, url); // Process CSS URLs
                dataToWrite = stripCssComments(dataToWrite); // Remove CSS comments
                dataToWrite = dataToWrite.replace(/\s+/g, ' ').trim(); // Minify CSS for optimization
            } else if (mimeType.includes('/html')) {
                dataToWrite = textContent.replace(/<!--[\s\S]*?-->/g, ''); // Remove HTML comments
            } else if (mimeType.includes('/javascript') || mimeType.includes('/js')) {
                dataToWrite = textContent.replace(/\/\/[^\n\r]*/g, ''); // Remove single-line comments
            } else if (mimeType.includes('/plain') || mimeType.includes('/xml')) {
                dataToWrite = textContent; // Handle plain text and XML
            } else {
                console.warn(`Unhandled text mime type: ${mimeType}`);
                dataToWrite = textContent;
            }
        } else if (mimeType.includes('image/')) {
            dataToWrite = Buffer.from(dataToWrite).toString('base64'); // Convert images to Base64
        } else if (mimeType.includes('application/')) {
            if (mimeType.includes('/json')) {
                try {
                    JSON.parse(dataToWrite.toString('utf8')); // Validate JSON
                } catch (err) {
                    console.error(`Invalid JSON content for ${url}:`, err.message);
                }
                dataToWrite = Buffer.from(dataToWrite).toString('utf8');
            } else if (mimeType.includes('/font') || mimeType.includes('/woff') || mimeType.includes('/ttf')) {
                dataToWrite = Buffer.from(dataToWrite).toString('base64'); // Encode fonts to Base64
            } else if (mimeType.includes('/pdf')) {
                dataToWrite = Buffer.from(dataToWrite).toString('base64'); // Encode PDFs to Base64
            } else {
                console.warn(`Unhandled application mime type: ${mimeType}`);
                dataToWrite = dataToWrite.toString('utf8'); // Default to UTF-8 encoding
            }
        } else if (mimeType.includes('audio/') || mimeType.includes('video/')) {
            dataToWrite = Buffer.from(dataToWrite).toString('base64'); // Encode media files to Base64
        } else {
            console.warn(`Unhandled mime type: ${mimeType}`);
            dataToWrite = dataToWrite.toString('utf8'); // Default handling for unknown types
        }
